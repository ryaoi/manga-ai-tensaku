...
【タスク】
添削済み画像（1枚目）に描かれている赤い修正線の位置に、注釈テキストをパーセンテージ座標付きで配置してください。
...
- annotationsは3〜8個`;
    ...
    return {
      annotations: (parsed.annotations || []).map((a: { x: number; y: number; text: string; category: TextAnnotation['category']; anchor?: TextAnnotation['anchor'] }) => {
  ...
  const generateFeedback = async () => {
    ...
    try {
      ...
      // Step 4: Done — start with all annotations collapsed (dots only)
      setLoadingStep(4);
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="text-center space-y-4">
          <div className="flex justify-center gap-2">
            <Link to="/hairstyle" className={buttonVariants({ variant: 'outline', size: 'sm' })}>
              髪型課題
            </Link>
            <Link to="/cleanup" className={buttonVariants({ variant: 'outline', size: 'sm' })}>
              添削除去ページへ
            </Link>
          </div>
